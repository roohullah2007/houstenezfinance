...
export default function CreateRealEstateListing() {
    ...
    function handleFiles(files: File[]) {
        ...
        setPreviewImages((prev) => [...prev, ...imgs.map((f) => URL.createObjectURL(f))]);
    }
    ...
    function handleSubmit(e: FormEvent) {
        e.preventDefault();
        post('/admin/real-estate-listings', { forceFormData: true });
    }

    return (
        <>
            <Head title="Create Property Listing" />
            <div className="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
                <div className="flex items-center gap-3">
                    <Link href="/admin/real-estate-listings" className="rounded-lg border border-gray-300 p-2 hover:bg-gray-100">
                        <ArrowLeft className="h-4 w-4 text-gray-600" />
                    </Link>
                    <h1 className="text-xl font-semibold text-gray-900">Create Property Listing</h1>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <section className="rounded-xl bg-white p-6 shadow-sm">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            <div className="sm:col-span-2 lg:col-span-3">
                                ...
                                <input type="text" className={inputClass} placeholder="e.g., Beautiful Home in Great Neighborhood" value={data.title} onChange={(e) => setData('title', e.target.value)} />
                                {errors.title && <p className="mt-1 text-xs text-red-500">{errors.title}</p>}
                            </div>
                            <div>
                                <label className={labelClass}>Developer / Builder</label>
                                <input type="text" className={inputClass} placeholder="e.g., ABC Homes" value={data.developer} onChange={(e) => setData('developer', e.target.value)} />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <select className={selectClass} value={data.listing_type} onChange={(e) => setData('listing_type', e.target.value)}>
                            ...
                            <div>
                                ...
                                <input type="number" min={0} step={1000} className={inputClass} placeholder="299000" value={data.price} onChange={(e) => setData('price', e.target.value)} />
                    ...
                    <section className="rounded-xl bg-white p-6 shadow-sm">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            <div className="sm:col-span-2 lg:col-span-3">
                                ...
                                <input type="text" className={inputClass} placeholder="123 Main Street" value={data.address} onChange={(e) => setData('address', e.target.value)} />
                                {errors.address && <p className="mt-1 text-xs text-red-500">{errors.address}</p>}
                            </div>
                            <div>
                                <label className={labelClass}>City *</label>
                                <input type="text" className={inputClass} placeholder="Houston" value={data.city} onChange={(e) => setData('city', e.target.value)} />
                            ...
                            <div>
                                ...
                                <input type="text" className={inputClass} placeholder="77082" value={data.zip} onChange={(e) => setData('zip', e.target.value)} />
                                {errors.zip && <p className="mt-1 text-xs text-red-500">{errors.zip}</p>}
                            </div>
                            <div>
                                <label className={labelClass}>Subdivision</label>
                                <input type="text" className={inputClass} placeholder="Optional" value={data.subdivision} onChange={(e) => setData('subdivision', e.target.value)} />
                            </div>
                            <div className="sm:col-span-2 lg:col-span-3">
                                <p className="mb-2 text-sm font-medium text-gray-700">Location coordinates (optional)</p>
                                <p className="mb-3 text-xs text-gray-500">Adding lat/long shows a map pin on the public detail page.</p>
                                <div className="grid gap-3 sm:grid-cols-2">
                                    <div>
                                        <label className={labelClass}>Latitude</label>
                                        <input type="text" className={inputClass} placeholder="e.g., 29.7604" value={data.latitude} onChange={(e) => setData('latitude', e.target.value)} />
                                        {errors.latitude && <p className="mt-1 text-xs text-red-500">{errors.latitude}</p>}
                                    </div>
                                    <div>
                                        <label className={labelClass}>Longitude</label>
                                        <input type="text" className={inputClass} placeholder="e.g., -95.3698" value={data.longitude} onChange={(e) => setData('longitude', e.target.value)} />
                    ...
                    <section className="rounded-xl bg-white p-6 shadow-sm">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            <div>
                                ...
                                <input type="number" min={0} max={20} className={inputClass} placeholder="3" value={data.bedrooms} onChange={(e) => setData('bedrooms', e.target.value)} />
                                {errors.bedrooms && <p className="mt-1 text-xs text-red-500">{errors.bedrooms}</p>}
                            </div>
                            <div>
                                <label className={labelClass}>Full Bathrooms *</label>
                                <input type="number" min={0} max={20} className={inputClass} placeholder="2" value={data.full_bathrooms} onChange={(e) => setData('full_bathrooms', e.target.value)} />
                                {errors.full_bathrooms && <p className="mt-1 text-xs text-red-500">{errors.full_bathrooms}</p>}
                            </div>
                            <div>
                                <label className={labelClass}>Half Bathrooms</label>
                                <input type="number" min={0} max={20} className={inputClass} placeholder="1" value={data.half_bathrooms} onChange={(e) => setData('half_bathrooms', e.target.value)} />
                            </div>
                            <div>
                                <label className={labelClass}>Square Feet *</label>
                                <input type="number" min={0} className={inputClass} placeholder="2000" value={data.square_feet} onChange={(e) => setData('square_feet', e.target.value)} />
                            </div>
                            <div>
                                <label className={labelClass}>Lot Size (Sq Ft)</label>
                                <input type="number" min={0} className={inputClass} placeholder="e.g., 7500" value={data.lot_size} onChange={(e) => setData('lot_size', e.target.value)} />
                            </div>
                            <div>
                                <label className={labelClass}>Year Built</label>
                                <input type="number" min={1800} max={2030} className={inputClass} placeholder="2010" value={data.year_built} onChange={(e) => setData('year_built', e.target.value)} />
                            </div>
                        </div>
                    </section>
                    ...
                    <section className="rounded-xl bg-white p-6 shadow-sm">
                        <div className="mb-4 flex items-center justify-between">
                            ...
                            <span className="text-xs text-gray-500">{previewImages.length} / 50 photos</span>
                        </div>
                        <p className="mb-4 text-sm text-gray-600">
                            Upload up to 50 photos and select your main (front) photo. Click the star icon on any photo to set it as the main listing photo.
                        </p>
                                ...
                                {previewImages.map((src, i) => {
                                    ...
                                    return (
                                        <div key={i} className={`group relative aspect-[4/3] overflow-hidden rounded-lg border-2 ${isMain ? 'border-[#F26B5E]' : 'border-gray-200'}`}>
                                            ...
                                            {isMain && (
                                                <div className="absolute top-1 left-1 rounded-full bg-[#F26B5E] px-2 py-0.5 text-[10px] font-semibold text-white">MAIN</div>
                                            )}
                                            <button
                                                type="button"
                    ...
                    <div className="flex justify-end gap-3">
                        <Link href="/admin/real-estate-listings" className="rounded-lg border border-gray-300 px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50">Cancel</Link>
...
CreateRealEstateListing.layout = {
    breadcrumbs: [
        { title: 'Dashboard', href: '/admin/dashboard' },
        { title: 'Real Estate Listings', href: '/admin/real-estate-listings' },
        { title: 'Create', href: '/admin/real-estate-listings/create' },
    ],
};
